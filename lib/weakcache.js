...
export default class WeakCache {
  ...
  set(key, value) {
    ...
    if (currentEntry) {
      this._totalCacheSize -= currentEntry.getDataSize();
      this._cache.delete(key);
    }
    if (value !== undefined) {
      const newEntry = new CacheEntry(value, this);
      if (newEntry.getDataSize() <= this._maxSize) {
        ...
        this._totalCacheSize += newEntry.getDataSize();
        this._removeStaleEntries();
        newEntry.removeImmune();
      }
    }
  }
    ...
    while (this._totalCacheSize > this._maxSize) {
      ...
      for (let [key, cacheEntry] of this._cache) {
        ...
        if (lowestAge === null) {
          ...
          lowestAge = Math.min(lowestAge, cacheEntry.getAge());
        }
      }
      if (this._totalCacheSize <= this._maxSize) {
        break;
